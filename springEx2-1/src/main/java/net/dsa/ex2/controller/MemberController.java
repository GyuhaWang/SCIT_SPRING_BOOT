...
public class MemberController {
	...
	@GetMapping("login")
	public String login(
			@CookieValue(name = "recentId", defaultValue = "")
				String recentId,
			Model model
			) {
	...
	public String loginForm(
			...
			@RequestParam(name = "check", defaultValue = "false")
				boolean check,
			HttpServletResponse response
			) {
		log.debug("> 로그인 데이터: id={}, pw={}, check={}"
				, id, pw, check);
		...
		if (result) {
			...
			String savedId = (String) session.getAttribute("loginId");
			log.debug("> 로그인 성공! 현재 세션ID: {}", savedId);
			...
			// 로그인 페이지에서 쿠키값을 사용하기 위한 redirect 요청경로
